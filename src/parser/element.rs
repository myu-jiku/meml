...
impl Element {
    ...
    pub fn construct(pair: Pair<Rule>, local_definitions: &DefinitionMap, function_arguments: Option<&Arguments>) -> Self {
    ...
    pub fn eval_contents(&mut self, mut pairs: Pairs<Rule>, local_definitions: &DefinitionMap, function_arguments: Option<&Arguments>) {
        for attribute in pairs.next().unwrap().into_inner() {
            ...
            self.arguments.push((
                ...
                parse_string(inner_rules.next().unwrap(), local_definitions, function_arguments),
            ));
        }
        
        for child in pairs.next().unwrap().into_inner() {
            match child.as_rule() {
                Rule::element => self.children.push(Element::construct(child, local_definitions, function_arguments)),
                Rule::const_use => {
                    if let Some(value) = local_definitions.get("elements").unwrap().get(child.as_str()) {
                ...
                Rule::func_use => {
                    ...
                    let args = inner_rules.next().unwrap().into_inner().map(|pair| parse_string(pair, local_definitions, None)).collect();
    ...
    pub fn as_xml(&self) -> String {
        ...
        let arguments = self.arguments.iter().map(|item| {
